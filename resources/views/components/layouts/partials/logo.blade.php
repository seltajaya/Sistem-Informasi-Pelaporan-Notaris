...
@php
    $hasLogo = file_exists(public_path('images/logo-kemenkum.png'));
...
<div class="flex items-center gap-3">
    @if ($hasLogo)
        <img src="{{ asset('images/logo-kemenkum.png') }}"
            alt="Logo Kementerian Hukum dan HAM RI"
            class="{{ $compact ? 'h-12 w-12' : 'h-14 w-14' }} shrink-0 object-contain">
    @else
        {{-- Placeholder mark; drop the official logo at public/images/logo-kemenkum.png --}}
        <div class="{{ $compact ? 'h-12 w-12' : 'h-14 w-14' }} shrink-0 rounded-xl bg-kumham-800 ring-1 ring-white/20 flex items-center justify-center shadow-card">
            <svg class="{{ $compact ? 'h-7 w-7' : 'h-8 w-8' }}" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M12 3v3m0 0c-.8 0-1.6.4-2.2 1-.6.6-1 1.4-1 2.2v1h6.4v-1c0-.8-.4-1.6-1-2.2-.6-.6-1.4-1-2.2-1Zm0 0V5.5m-3.2 3.7h6.4M6.4 13l-1.2 2.4h13.6L17.6 13M7.2 15.4 12 21l4.8-5.6"
                    stroke="#D4AF37"
                    stroke-width="1.4"
